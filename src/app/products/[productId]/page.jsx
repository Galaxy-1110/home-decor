...
export default function Productdetails({ params }) {
	...
	const handleBtnClick = async () => {
		...
		animate(".icon-pop", { left: ["-15px", "auto"] }, { duration: 0.4 });
		await animate(".text", { opacity: [0, 1] }, { duration: 0.2 });

		await setTimeout(async () => {
			animate(".icon", { opacity: [0, 1] }, { duration: 0.2, delay: 0.4 });
			animate(".icon", { x: [120, 0] }, { duration: 0.8, delay: 0.4 });
			await animate(".text", { opacity: [1, 0] }, { duration: 0.6 });
						...
						<div className=" flex flex-col gap-12 ">
							...
							<div ref={scope}>
								<motion.button
									...
									className="btn px-8 py-4 bg-[#6D583F] text-[#F5E9DB] rounded-3xl flex gap-5 align-middle items-center relative overflow-hidden">
									<IoCheckmarkCircle className="icon-pop absolute top-auto left-[-15px]" />
